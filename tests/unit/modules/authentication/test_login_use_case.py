...
def _build_use_case(user_repository, password_hasher, refresh_token_repository, token_service, clock, unit_of_work) -> LoginUseCase:
    return LoginUseCase(
...
async def test_login_with_valid_credentials_returns_token_pair(
    user_repository, password_hasher, refresh_token_repository, token_service, clock, unit_of_work
) -> None:
    ...
    output = await use_case.execute(LoginInput(email="user@example.com", password="s3cr3t!!"))
