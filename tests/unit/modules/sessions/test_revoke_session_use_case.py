...
async def test_revoke_session_marks_it_as_revoked(session_repository, clock, unit_of_work) -> None:
    user_id = uuid4()
    session = Session.start(user_id=user_id, started_at=clock.now())
    await session_repository.add(session)
    use_case = RevokeSessionUseCase(session_repository, clock, unit_of_work)
...
async def test_revoke_unknown_session_raises_not_found(session_repository, clock, unit_of_work) -> None:
    use_case = RevokeSessionUseCase(session_repository, clock, unit_of_work)
...
async def test_revoke_session_belonging_to_another_user_raises_not_found(
    session_repository, clock, unit_of_work
) -> None:
    owner_id = uuid4()
    attacker_id = uuid4()
...
async def test_revoke_already_revoked_session_is_idempotent(session_repository, clock, unit_of_work) -> None:
    user_id = uuid4()
    session = Session.start(user_id=user_id, started_at=clock.now())
